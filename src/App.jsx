...
const HI = {
  ...
  "Ambria Work Force":"अम्ब्रिया वर्क फ़ोर्स",
  ...
  "Ambria Work Force":"अम्ब्रिया वर्क फ़ोर्स","Employee ID":"कर्मचारी आईडी",
...
function DeptView({attendance, setAttendance, events, kitchenTracking, setKitchenTracking, lang="en", setLang, onSelectDept, onLogout, currentUser, forceDept, leaves, setLeaves, empDb, setEmpDb}) {
  ...
  return (
    <div style={{padding:"4px 0"}}>
      ...
      {activeTab==="repair"&&<RepairMaintenance lang={lang}/>}
  ...
  return (
    <div>
      ...
      {(()=>{
        ...
        const openRepairs = 3; // static count of urgent repairs from sample data
        return(
          ...
          <div>
            ...
            {allSecs.map(sec=>{
              ...
              const allDishNames = [...new Set([
                ...(d1View!=="new"?contItems.map(d=>d.name):[]),
                ...(d1View!=="cont"?newItems.map(d=>d.name):[]),
              ])];
              ...
              return(
                <Card key={sec} style={{marginBottom:10,padding:0,overflow:"hidden"}}>
                  ...
                  <div onClick={()=>toggleSec("d1sec_"+sec)} style={{padding:"12px 16px",background:m2.color+"12",cursor:"pointer",borderBottom:secOpen?`1px solid ${C.border}`:"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:11,color:C.amber}}>{contItems.filter(d=>ds(d.fEvId,d.fIdx).mesaDone).length}/{contItems.length}</span>
                      ...
                      return(
                        <div key={dishName} style={{marginBottom:6}}>
                          ...
                          <div onClick={()=>setExpandedDish(isExp?null:cKey)} style={{cursor:"pointer",borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
                            ...
                            {/* Side-by-side pax columns */}
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0}}>
                              {/* Left: Continue D-1 */}
                              <div style={{padding:"8px 12px",background:cDish?C.amberBg+"40":"transparent",borderRight:`1px solid ${C.border}`}}>
                                {cDish?(
                                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                                    ...
                                    <div>
                                      ...
                                      <div style={{fontSize:9,color:C.faint}}>Jun 1</div>
                                    </div>
                                  </div>
                                ...
                                {nDish?(
                                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                                    ...
                                    <div>
                                      ...
                                      <div style={{fontSize:9,color:C.faint}}>Jun 2</div>
                                    </div>
                                  </div>
                                ):<div style={{fontSize:10,color:C.faint}}>—</div>}
                              </div>
                          ...
                          {isExp&&(
                            <div style={{padding:"8px 12px",borderRadius:"0 0 10px 10px",background:C.surface,border:`1px solid ${C.border}`,borderTop:"none"}}>
                              ...
                              {/* Mark all done buttons */}
                              <div style={{display:"flex",gap:6,marginTop:8,flexWrap:"wrap"}}>
                                {cDish&&!cDone&&<button onClick={e=>{e.stopPropagation();setDs(cDish.fEvId,cDish.fIdx,{mesaDone:true});}} style={{flex:1,padding:"8px",borderRadius:8,background:`linear-gradient(135deg,${C.amber},#A05010)`,color:"#fff",border:"none",fontSize:11,fontWeight:700,cursor:"pointer",minHeight:36}}>✅ Jun 1 — {cDish.totalPax} pax</button>}
                                {nDish&&!nDone&&<button onClick={e=>{e.stopPropagation();setDs(nDish.fEvId,nDish.fIdx,{mesaDone:true});}} style={{flex:1,padding:"8px",borderRadius:8,background:`linear-gradient(135deg,${C.gold},#A8891E)`,color:"#0A0908",border:"none",fontSize:11,fontWeight:700,cursor:"pointer",minHeight:36}}>✅ Jun 2 — {nDish.totalPax} pax</button>}
                              </div>
                            </div>
                          )}
                        </div>
